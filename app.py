...
# ----------------- PAGE 2: MAIN DASHBOARD SOFTWARE -----------------
    ...
    with tab1:
        ...
        if st.button("Save Customer", type="primary"):
            if cust_name: st.success(f"'{cust_name}' ka naam Cloud par save ho gaya!")
            else: st.warning("Kripya naam likhein!")
    ...
    with tab2:
        ...
        with col1:
            if st.button("➕ Add Income", use_container_width=True): st.success(f"Rs. {amount} Income me add ho gaye!")
        with col2:
            if st.button("➖ Add Expense", use_container_width=True): st.error(f"Rs. {amount} Expense me add ho gaye!")
